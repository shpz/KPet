/**
 * 用户名与管道名（§4.1：事件管道 \\.\pipe\KimiPet.H2D.<用户名>，管道名不允许 \）。
 */
...
/** 用户名 → 管道名段中的非法字符：\ 为管道名硬限制（§4.1），其余按路径/文件系统保留字符一并替换，避免歧义与安全隐患。 */
...
/** 事件管道全名：\\.\pipe\KimiPet.H2D.<用户名>（§4.1）。 */
...
/** 控制管道全名：\\.\pipe\KimiPet.PET.<用户名>（§4.1，守护进程 ↔ 渲染进程双向）。 */
