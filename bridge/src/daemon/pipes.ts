 ...
 * - 单行超过 2×64KB（消息上限的两倍，§4.2）视为异常帧：丢弃缓冲并计错误。
 */
...
/** 单行缓冲上限：协议单条消息 64KB（§4.2），留一倍余量后仍超则判异常帧。 */
export const MAX_LINE_BYTES = MAX_MESSAGE_BYTES * 2;

/**
 * 给单个 socket 挂接行分帧（§4.1/§4.2 + 线上约定）：
