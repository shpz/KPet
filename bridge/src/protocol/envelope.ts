 ...
 * - payload：消息体，字段与 §4.3 表一一对应
 *
 * 编码 UTF-8；一条管道消息 = 一个完整 JSON 对象（单条上限 64KB，§4.2）。
 */
...
export interface CreateEnvelopeOptions {
  /** 消息唯一标识；缺省自动生成 UUID（§4.2 中 id 为可选字段）。 */
  id?: string;
...
/** 构造一个协议信封（§4.2）。id 缺省自动生成，便于去重与日志关联。 */
...
/**
 * 从宿主事件对象提取 session_id（宿主事件基础字段之一，§3.1）。
 * 缺失或非字符串一律返回 null —— 转发器不依赖任何具体字段（§3.4）。
 */
...
/**
 * 把宿主事件原始 JSON 包装成 host_event 信封（§3.3 / §4.3）：
 * - payload._raw：宿主事件 JSON 的原始文本，整体透传（§3.4），不解析、不重排、不做任何语义处理
...
/**
 * 校验一个反序列化后的对象是否为合法信封（§4.2 / §4.4）。
 *
 * 规则：
 * - v 必须为 1；type 必须为非空字符串（未知类型视为结构合法，由收方按 §4.2「忽略并记日志」处理）；
 * - ts 必须为可解析的时间字符串；session_id 必须为 string 或 null；payload 必须为普通对象；
 * - id 可选（§4.2）；字段级只增不改，未知字段一律忽略（§4.4）。
 *
 * payload 内字段不做深校验：解析端按 §4.4「缺失取默认值、未知忽略」处理。
 */
