...
# 也透传 --stop 等其他参数（部署脚本用 `sh kimi-pet-relay.sh --stop` 请求旧版守护进程优雅退出）。

# 脚本固定位于 <插件根>/bin/ 下，据此向上解析插件根（宿主钩子工作目录即插件根，但以脚本自身位置为准更稳）。
SCRIPT_DIR=$(CDPATH= cd "$(dirname "$0")" && pwd)
PLUGIN_ROOT=$(dirname "$SCRIPT_DIR")

# 定位 Windows 侧 kimi-petd.exe。注意必须用 Linux 路径交给 exec：
...
if [ -n "${KIMI_PLUGIN_ROOT_WIN:-}" ]; then
  if ! command -v wslpath >/dev/null 2>&1; then
    echo "kimi-pet-relay.sh: 未找到 wslpath（需要 WSL 环境）" >&2
    exit 1
  fi
  PLUGIN_ROOT_LINUX=$(wslpath -u "$KIMI_PLUGIN_ROOT_WIN") || {
    echo "kimi-pet-relay.sh: wslpath 转换失败: $KIMI_PLUGIN_ROOT_WIN" >&2
    exit 1
  }
  DAEMON_EXE="$PLUGIN_ROOT_LINUX/bin/kimi-petd.exe"
else
  DAEMON_EXE="$PLUGIN_ROOT/bin/kimi-petd.exe"
fi
