# KPet 跨平台兼容方案：WSL 与 macOS

> 范围：本文只做调研与方案设计，不包含任何代码改动。所有结论均基于当前仓库真实代码并标注 `path:line` 引用。
>
> 现状定位：KPet 当前是「Windows 单平台」产品。守护进程（`kpetd`）与转发器是 Node.js 22 产物，渲染端是 UE5 C++ 的 Win32 分层窗口。本方案梳理其中所有 Windows 专属假设，并分别给出 WSL（两种形态）与 macOS 的落地路径。
...
- 事件/控制管道名硬编码为 Windows 命名管道：
  - `bridge/src/bridge/user.ts:39-48`：`getEventPipeName()` → `\\.\pipe\KPet.H2D.<用户名>`；`getControlPipeName()` → `\\.\pipe\KPet.PET.<用户名>`。
...
- 管道服务端（守护进程侧）：
  ...
  - `bridge/src/daemon/control.ts:2`：控制管道注释 `\\.\pipe\KPet.PET.<用户名>`。
- 单实例用「管道名占用」实现：
...
- 用户配置目录：`KIMI_CODE_HOME` 未设回退 `~/.kimi-code`：
  - `bridge/src/daemon/config.ts:43-56` `getKPetHome`；注释里 Windows 默认 `C:\Users\<用户名>\.kimi-code`。
...
- 硬编码 `.exe` 与 `kpetd.exe`：
  - `bridge/src/bridge/daemon.ts:18` `DAEMON_EXE = 'kpetd.exe'`。
  - `bridge/src/bridge/daemon.ts:45-53` `resolveDaemonPath`：`/\.exe$/i` 判定自身；`KIMI_PLUGIN_ROOT/bin/kpetd.exe`；`cwd()/bin/kpetd.exe`。
...
- 插件清单命令用 Windows 反斜杠相对路径：
  - `bridge/packaging/kpet/kimi.plugin.json:6-17`：每条 hook 的 `command` 都是 `.\bin\kpetd.exe --relay`。
...
- 管道名拼装用 `GetUserNameW`：
  - `PetControlClient.cpp:729-750` `BuildPipeName`：`GetUserNameW` → `USERNAME` → `USER`，拼 `\\.\pipe\KPet.PET.<用户名>`。
- 显示器 id 用 `MonitorFromPoint` / `GetMonitorInfoW`：
  - `PetControlClient.cpp:134-147` `SendPetMoved`。

#### 2.2.3 会话面板（仅 WebUI 路径；UMG 路径已移除）

- 会话面板仅 WebUI 路径（UMG 路径已移除、无降级），依赖 CEF（`SWebBrowser`，见 `Pet/Source/Pet/Private/UI/PetSessionWebPanel.cpp`）：CEF 在 macOS 无现成对应物，需另行处理（WebKit/CEF 等价物），因此会话面板并非「Mac 可直接复用」。
- 会话面板的窗口宿主是标准 Slate `SWindow`、不依赖 Win32（跨平台部分仅限窗口承载）：
...
- `tools/mock-daemon.ts:35`：`\\.\pipe\KPet.PET.<用户名>` 命名管道。
- `tools/package.ts`：整包打包全 Windows：
  ...
  - `tools/package.ts:47` `GAME_EXE_NAMES = ['pet.exe','kpet.exe']`；`256` 产出 `bin/kpetd.exe`。
  - `tools/package.ts:160-166` `bun build --compile --windows-hide-console`。
- `bridge/package.json:12` `build:exe`：`bun build --compile --windows-hide-console dist/launcher/main.js --outfile bin/kpetd.exe`。
...
> **WSL2 interop（默认开启）允许在 WSL 内直接启动 Windows 可执行文件**，例如 `/mnt/c/.../kpetd.exe` 或 `wt.exe`、`wslpath.exe`。Windows 进程被拉起后，其运行环境是 Windows（能连 Windows 命名管道、能拉起 `Pet.exe`），但入参/工作目录/cwd 字符串仍是 Linux 语义，需要显式转换。

### 3.1 形态一：CLI 会话在 WSL、守护进程仍在 Windows（推荐）

此形态下：kimi CLI 跑在 WSL 发行版里；守护进程 `kpetd --daemon` 与渲染端 `Pet.exe` 继续跑在 Windows。转发器需要能「从 WSL 把事件交到 Windows 守护进程」。

**核心问题有三个：**

1. **插件 hook 命令在 WSL 里是 `.\bin\kpetd.exe --relay`，这个命令无法直接执行**（bash 不认反斜杠，且 Linux 里没有 `.exe` 语义）。见 `bridge/packaging/kpet/kimi.plugin.json:6-17`。
...
- hook 命令改为跨发行版的互操作写法（在 WSL 内能定位到 Windows 插件目录）。两种可行做法：
  - （推荐）插件清单增加 WSL 专用清单（或运行时探测），command 用 `/mnt/<drive>/<path>/bin/kpetd.exe --relay`（把 `.\bin\...` 换成 interop 绝对路径）。
...
| 文件 | 现状（引用） | 改造 |
|------|-------------|------|
| `bridge/packaging/kpet/kimi.plugin.json:6-17` | command 为 `.\bin\kpetd.exe --relay` | 增加 WSL 兼容的 command（interop 绝对路径）或新增 WSL 清单 |
...
| `bridge/src/bridge/daemon.ts:18,45-53` | 硬编码 `.exe` / `kpetd.exe` | 保留 Windows 产物即可（形态一下守护进程仍是 Windows exe，无需改） |
| `bridge/src/bridge/user.ts:29-37` | 用户名取 `os.userInfo()`/`USERNAME` | 形态一下无需改（转发器是 Windows 进程） |

### 3.2 形态二：守护进程整体跑在 WSL 内

此形态下：`kpetd --daemon` 以 Linux 进程跑在 WSL 里；渲染端 `Pet.exe` 仍在 Windows。

**核心问题：**

1. **事件/控制管道跨 WSL 边界**。Linux 进程不能直接 `CreateFileW` 连 `\\.\pipe\...`，也不能让 Windows `Pet.exe` 直接连 Linux 的 Unix domain socket。需要一个跨边界的传输层。
   - Node 侧 `node:net` 已经抽象了 `listen(path)`/`connect(path)`（`pipes.ts:63-70`、`pipe.ts:22-83`），把路径从 `\\.\pipe\...` 换成 Unix domain socket 路径即可（如 `$XDG_RUNTIME_DIR/kpet-<uid>.sock` 或 `/tmp/kpet-<uid>.sock`），**Node 侧几乎零改动**。
...
3. **KIMI_CODE_HOME 与路径**。守护进程在 WSL 内读到的 `KIMI_CODE_HOME` 是 Linux 路径（`~/.kimi-code` 在 WSL 用户主目录），而渲染端/Pet.exe 在 Windows 读的又是 Windows 侧目录。两边的会话目录、位置缓存、抑制标记（`pet.disabled`）会出现双份/不同步：
   ...
   - `PetControlClient.cpp:34-49` `WriteCloseSuppressionMarker` 写的是 Windows `%TEMP%/kpet/pet.disabled`，与 WSL 内守护进程的 `%TEMP%`（Linux `/tmp`）不一致，会导致「用户关闭标记」失配。
...
| `bridge/src/daemon/petstate.ts:10-12` | `%APPDATA%` | WSL 下改 `XDG_DATA_HOME` 或统一到 `KIMI_CODE_HOME/kpet` |
| `bridge/src/bridge/daemon.ts:18,45-53` | `kpetd.exe` | 平台产物名 `kpetd`（Linux ELF） |
...
| 模块 | 现状（引用） | macOS 改造 |
|------|-------------|-----------|
| 事件/控制管道 | `bridge/src/bridge/user.ts:39-48`、`bridge/src/bridge/pipe.ts:13-14,22-83`、`bridge/src/daemon/pipes.ts:63-70` | `node:net` 换 Unix domain socket 路径（如 `$TMPDIR/kpet-<uid>.sock` 或 `~/.kimi-code/kpet/*.sock`）；`EADDRINUSE` 单实例语义在 UDS 下仍是「socket 文件已存在」，但需在启动前清理 stale socket 文件 |
...
| 位置缓存 | `bridge/src/daemon/petstate.ts:10-12` | `APPDATA` 换成 `~/Library/Application Support/KPet/pet-state.json` |
...
| 可执行产物 | `bridge/src/bridge/daemon.ts:18,45-53`、`bridge/src/launcher/main.ts:1-25` | `bun build --compile`（不带 `--windows-hide-console`）产出 Mach-O `kpetd`；`.exe` 判定改为平台化 |
...
|------|----------------|--------------|
| bridge 产物 | `bin/kpetd.exe`（`bridge/package.json:12`） | `bin/kpetd`（Mach-O，`bun build --compile` 不带 `--windows-hide-console`） |
...
| 插件清单 | `.\bin\kpetd.exe --relay`（`kimi.plugin.json:6-17`） | `./bin/kpetd --relay`（正斜杠，无 `.exe`） |
...
3. `bridge/src/bridge/daemon.ts:18,45-53`、`bridge/src/launcher/main.ts:23`：`kpetd.exe` 判定改为平台化产物名（Windows 保留现状，其余平台 `kpetd`）。
